{{--@dd(\Morilog\Jalali\Jalalian::forge($class->start_date)->format('Y/m/d'))--}}
    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">@yield('title') <a href="{{ route('classes.index') }}"
                                                          class="btn btn-primary">لیست کلاس ها</a></h3>
                ...
                <form action="{{ route('classes.update',$class) }}" method="POST">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="teacher" id="" class="form-control select2">
                                @forelse($teachers as $teacher)
                                    <option
                                        value="{{ $teacher->id }}" {{ $class->teacher_id == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->fullName() }}</option>
                                @empty
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="university" id="" class="form-control select2">
                                @forelse($universities as $uni)
                                    <option
                                        value="{{ $uni->id }}" {{ $class->university_id == $uni->id ? 'selected' : '' }}>{{ $uni->name }}</option>
                                @empty
                    ...
                    <div class="row p-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" value="{{ $class->name }}" id="" class="form-control"
                    ...
                    <div class="row p-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="boys" value="{{ $class->boys ?? '0' }}" id="" class="form-control">
                        </div>
                        <div class="col-md-6">
                            <label for="">تعداد دانشجویان دختر</label>
                            <input type="number" name="girls" value="{{ $class->girls ?? '0' }}" id="" class="form-control">
                        </div>
                    </div>
